...
namespace Bawx
{
    public static class GreedyMesh
    {
        ...
        public static void Generate<T>(byte[][][] grid, Func<int[], int[], int[], int, VoxelFace, bool, IEnumerable<T>> createQuad, 
            out T[] vertices, out int[] indices, bool cw = false) where T : IVertexType
        {
            ...
            for (var direction = 0; direction < 3; direction++)
            {
                ...
                for (pos[direction] = -1; pos[direction] < grid.GetSize(direction); pos[direction]++)
                {
                    ...
                    if (!noFront)
                        ProcessLayer(frontfaces, createQuad, verts, inds, pos[direction], direction, size, cw);
                    pos[u]++;
                    if (!noBack)
                        ProcessLayer(backfaces, createQuad, verts, inds, pos[direction], direction, size, !cw);
                }
            }
        ...
        private static void ProcessLayer<T>(
            VoxelFace[] faces, Func<int[], int[], int[], int, VoxelFace, bool, IEnumerable<T>> createQuad, 
            List<T> vertices, List<int> indices,
            int depth, int dir, int[] size, bool cw) where T : IVertexType
        {
            var u = (dir + 1)%3;
            var v = (dir + 2)%3;
            ...
            for (var v0 = 0; v0 < size[v]; v0++)
            {
                for (var u0 = 0; u0 < size[u]; u0++)
                {
                    ...
                    vertices.AddRange(createQuad(pos, du, dv, dir, currentFace, cw));
        ...
        private static Vector3 DirToUnitVector(int dir)
        {
            switch (dir)
            {
                case 0:
